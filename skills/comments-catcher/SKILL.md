...
2. 节奏与抽样参数按“命令行参数 > 技能目录 `config.json` > 内置默认”的优先级取值；内置默认对应 `--with-sub --sub-rate 0.5 --seed 42 --delay 5 --jitter-range 0.6`。实现允许的最小基础间隔是 3 秒；用户要求更慢时照做。
...
5. 用绝对脚本路径和参数数组调用采集器，例如：
   ...
   python <skill-root>/scripts/comments_catcher.py \
     ...
     --output <OUTPUT.json> --with-sub --delay 5 --sub-rate 0.5 --seed 42
   ```
...
技能目录下的 `config.json` 用于长期调控采集节奏与抽样，支持四个键：`delay`（请求基础间隔秒数，最低 3.0）、`jitter_range`（间隔随机抖动总幅度，0.6 表示 ±30%）、`sub_rate`（二级回复线程抽样比例 0–1）、`seed`（确定性抽样种子）。以 `_` 开头的键是注释，加载时忽略。命令行参数始终优先于该文件；也可用 `--config <PATH>` 指定其他配置文件。采集开始时程序会打印最终生效的参数与配置来源。已有抽样决策的恢复文件要求 `sub_rate` 与 `seed` 与原任务一致，修改它们只对新任务生效。
